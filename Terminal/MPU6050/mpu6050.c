...
Illegal_Def     AccelerationeNomal;//急变速
mpu_msg_typedef mpu_msg;


//判断加速度变化
//返回值:0,正常运行;  1=急加速;  2=急减速;
//    其他,错误代码
...
void MPU_Process(void)
{
    ...
    case 2:
    {
        if( mpu_dmp_get_data( &mpu_msg.pitch, &mpu_msg.roll, &mpu_msg.yaw ) == 0 ) {
            MPU_Get_Accelerometer( &mpu_msg.aacx, &mpu_msg.aacy, &mpu_msg.aacz );	// 得到加速度传感器数据
//            MPU_Get_Gyroscope( &mpu_msg.gyrox, &mpu_msg.gyroy, &mpu_msg.gyroz );	// 得到陀螺仪数据
            ...
            if(mpu_R == 1) {
                ...
                if(JiaspeedNum >= 8) {
                    JiaspeedNum = 0;
#ifdef __DEBUG
                    XFS_WriteBuffer(xfs_auto, "急加速");
                    if(m_var.Flag_Debug_Enable == SET)
                        Debug_Printf(SET,
                                     "急加速:\r\n俯仰角%.7f,横滚角%.7f,航向角:%.7f\r\nx加速度%d,y加速度%d,z加速度:%d,R加速度:%d",
                                     mpu_msg.pitch,
                                     mpu_msg.roll,
                                     mpu_msg.yaw,
                                     mpu_msg.aacx,
...
#endif
                    //添加违章记录存储
                    TimeToCharArray(Sys_CalendarTime,AccelerationeNomal.stime);  // 记录违章开始时间
                    TimeToCharArray(Sys_CalendarTime,AccelerationeNomal.etime);  // 记录违章结束时间
                    AccelerationeNomal.ino = Acceleratione_enum;                 // 违章类型
                    MX25L128_Illegal_Write(&AccelerationeNomal);                 // 将违章数据写入Flash中
